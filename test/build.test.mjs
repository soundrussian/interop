...
describe("@soundrussian/interop", () => {
  it("captures initial browser values in closure", () => {
    ...
    expect(fakeWindow.AsgInterop.getCurrentUrl()).toBe("https://example.com/page");
    expect(fakeWindow.AsgInterop.getReferrer()).toBe("https://referrer.example.com/");
  ...
  it("fails fast for the CommonJS build outside a browser", () => {
    expect(() => require(path.join(rootDir, "dist", "index.cjs"))).toThrow(/window is not defined/);
  });
  ...
  it("initializes window.AsgInterop from the IIFE bundle with captured values", async () => {
    ...
    expect(sandboxWindow.AsgInterop.getCurrentUrl()).toBe("https://sandbox.example.com/");
    expect(sandboxWindow.AsgInterop.getReferrer()).toBe("https://origin.example.com/");
